...
def DrawMapFromCoords(canvas, wsi_object, coords, patch_size, vis_level, indices=None, verbose=1, draw_grid=True):
    ...
    for idx in range(total):
        ...
        patch_id = indices[idx]
        coord = coords[patch_id]
        patch = np.array(wsi_object.wsi.read_region(tuple(coord), vis_level, patch_size))
...
def StitchPatches(hdf5_file_path, downscale=64, draw_grid=False, bg_color=(0,0,0), alpha=-1):
...
def StitchCoords(hdf5_file_path, wsi_object, downscale=64, draw_grid=False, bg_color=(0,0,0), alpha=-1):
...
def SamplePatches(coords_file_path, save_file_path, wsi_object, 
    ...
    for idx in indices:
        coord = coords[idx]
        patch = wsi_object.wsi.read_region(coord, patch_level, tuple([patch_size, patch_size]))
        if custom_downsample > 1:
