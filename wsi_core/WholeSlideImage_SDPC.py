...
class WholeSlideImage_SDPC(object):
    ...
    def isInContours(cont_check_fn, pt, holes=None, patch_size=256):
        if cont_check_fn(pt):
            if holes is not None:
                return not WholeSlideImage_SDPC.isInHoles(holes, pt, patch_size)
            else:
                return 1
        return 0
    ...
    def clip_process_counters(self, fp_chunk_size, save_path_hdf5, save_path, cases, patch_level=0, patch_size=256, step_size=256, **kwargs):
        for idx, cont in enumerate(cases):
    ...
    def process_contour(self, cont, contour_holes, patch_level, save_path, patch_size = 256, step_size = 256,
        ...
        results = pool.starmap(WholeSlideImage_SDPC.process_coord_candidate, iterable)
        pool.close()
    ...
    def process_coord_candidate(coord, contour_holes, ref_patch_size, cont_check_fn):
        if WholeSlideImage_SDPC.isInContours(cont_check_fn, coord, contour_holes, ref_patch_size):
            return coord
        else:
            return None
