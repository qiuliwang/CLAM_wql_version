# internal imports
from wsi_core.WholeSlideImage_SDPC import WholeSlideImage_SDPC as WholeSlideImage
from wsi_core.wsi_utils_SDPC import StitchCoords
...
id_bags = os.listdir('/home/fsk/Code/masks/2')
idsss = []
for one in id_bags:
	t = one.split('.jpg')[0]
	idsss.append(t)
...
def stitching(file_path, wsi_object, downscale = 64):
	start = time.time()
	heatmap = StitchCoords(file_path, wsi_object, downscale=downscale, bg_color=(0,0,0), alpha=-1, draw_grid=False, multi = multi_per)
...
def seg_and_patch(source, save_dir, patch_save_dir, mask_save_dir, stitch_save_dir, 
	...
	# for i in range(50):
		...
		if auto_skip and os.path.isfile(os.path.join(patch_save_dir, slide_id + '.h5')):
			...
			continue
		if slide_id in idsss:

			# Inialize WSI
			full_path = os.path.join(source, slide)
			WSI_object = WholeSlideImage(full_path, multi_per)
